...
public class BluetoothLeService extends Service implements IBlueToothService{
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            String intentAction;
            if(newState == 2) {     //BluetoothProfile.STATE_CONNECTED--   连接成功
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            if(UUID.fromString(BluetoothLeService.Characteristic_uuid_TX).equals(characteristic.getUuid())) {
                MLog.e("发送广播 ACTION_DATA_AVAILABLE t透传数据--ffe1");
                BluetoothLeService.this.broadcastUpdate("com.example.bluetooth.le.ACTION_DATA_AVAILABLE", characteristic);
            } else if(UUID.fromString(BluetoothLeService.Characteristic_uuid_FUNCTION).equals(characteristic.getUuid())) {
                MLog.e("发送广播 ACTION_DATA_AVAILABLE1 无操作--ffe2");
                BluetoothLeService.this.broadcastUpdate("com.example.bluetooth.le.ACTION_DATA_AVAILABLE1", characteristic);
            }
    ...
    static {
        ...
        Service_uuid = "0000ffe0-0000-1000-8000-00805f9b34fb";  //主uuid
        Characteristic_uuid_TX = "0000ffe1-0000-1000-8000-00805f9b34fb";    //疑似发送，接收
        Characteristic_uuid_FUNCTION = "0000ffe2-0000-1000-8000-00805f9b34fb";  //连接成功后使用一次，发广播时无操作
    }
    ...
    @Override
    public int command(String g, boolean string_or_hex_data) {
        int ic = 0;
        ...
        if(data_len_20 > 0) {
            while(i < data_len_20) {
                ...
                //发送指令
                MLog.e("data_len_20 > 0 执行指令--ffe1");
        ...
        if(data_len_0 > 0) {
            ...
            MLog.e("data_len_0 > 0 执行指令--ffe1");
            BluetoothGatt gatts = this.mBluetoothGatt;
            BluetoothGattService  sd = this.mBluetoothGatt.getService(UUID.fromString(Service_uuid));
            var10 = this.mBluetoothGatt.getService(UUID.fromString(Service_uuid)).getCharacteristic(UUID.fromString(Characteristic_uuid_TX));
            var10.setValue(da);
            this.mBluetoothGatt.writeCharacteristic(var10);
            ic += data_len_0;
    ...
    public int get_connected_status(List<BluetoothGattService> gattServices) {
        ...
        String uuid = null;
//        String unknownServiceString = this.getResources().getString(2131099663);
//        String unknownCharaString = this.getResources().getString(2131099662);
    ...
    private void broadcastUpdate(String action, BluetoothGattCharacteristic characteristic) {
        Intent intent = new Intent(action);
        Log.d("getUuid", " getUuid = " + characteristic.getUuid().toString());
//        Log.d("getUuid", " getValue = " + characteristic.getValue());
//        Log.d("getUuid", " getPermissions = " + characteristic.getPermissions());
//        Log.d("getUuid", " getWriteType = " + characteristic.getWriteType());
//        Log.d("getUuid", " getWriteType = " + characteristic.getDescriptor(characteristic.getUuid()).toString());
//        Log.d("getUuid", " getWriteType = " + characteristic.getDescriptor(characteristic.getUuid()).getValue());
//        Log.d("getUuid", " getWriteType = " + characteristic.getDescriptor(characteristic.getUuid()).getPermissions());
    ...
    public boolean initialize() {
        ...
        if(this.mBluetoothAdapter == null) {
            Log.e(TAG, "Unable to obtain a FNBluetoothAdapter.");
            return false;
        } else {
            return true;
        }
    ...
    public boolean connect(String address) {
        if(this.mBluetoothAdapter != null && address != null) {
            ...
            Log.w(TAG, "FNBluetoothAdapter not initialized or unspecified address.");
            return false;
        }
    }
...
//            this.mBluetoothGatt.close();    //释放连接资源。否则，在多次尝试连接蓝牙设备之后很快就会超出限制，导致无法连接设备
        } else {
            Log.w(TAG, "FNBluetoothAdapter not initialized");
        }
    }
